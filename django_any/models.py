...
def any_file_field(field, **kwargs):
    ...
    def get_some_file(path):
        ...
        if files:
            ...
            tmp_file = open("{}/{}".format(path, "tmp.txt"), "w")
            tmp_file.write("auto generated by django_whatever")
            tmp_file.close()
            result_file = "tmp.txt"
        instance = field.storage.open("{}/{}".format(path, result_file)).file
